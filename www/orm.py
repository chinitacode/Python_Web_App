...
def log(sql, args=()):
    logging.info('SQL: %s' % sql)  # creates a level INFO logging message
...
async def create_pool(loop, **kw):
    ...
    __pool = await aiomysql.create_pool(
        host = kw.get('host', 'localhost'), # the second argument is the default value when not found in kw
...
'''
Model只是一个基类，要将具体的子类如User的映射信息读取出来需要通过metaclass：ModelMetaclass。
